...
```{r, class_proportions, echo=FALSE, include=FALSE}
...
rr <- ggplot(F2_table, aes(x = reorder(Animal.ID, range), y = mean)) +  geom_linerange(aes(ymin = min, ymax =max)) + ggtitle("F2 variation in IFD ranges") + theme(axis.title.x=element_blank(),
        axis.text.x=element_blank(),
        axis.ticks.x=element_blank())
rr

mm <- ggplot(F2_table, aes(x = reorder(Animal.ID, -min), y = mean)) +  geom_linerange(aes(ymin = min, ymax =max)) +ggtitle("F2 variation in Minimum IFD values") + theme(axis.title.x=element_blank(),
        axis.text.x=element_blank(),
        axis.ticks.x=element_blank())
mm

vv <- ggplot(F2_table, aes(x = reorder(Animal.ID, var), y = mean)) +  geom_linerange(aes(ymin = min, ymax =max)) + ggtitle("F2 variation in IFD variance") + theme(axis.title.x=element_blank(),
        axis.text.x=element_blank(),
        axis.ticks.x=element_blank())
vv
```